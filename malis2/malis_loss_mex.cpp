...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]){
	...
	vector<mwIndex> nHood(nhood_dims[0]);
	for (mwIndex i=0; i<nhood_dims[0]; ++i) {
		nHood[i] = 0;
		for (mwIndex j=0; j<nhood_dims[1]; ++j) {
			nHood[i] += (mwIndex)nhood_data[i+j*nhood_dims[0]] * prodDims[j];
		}
	}
    ...
    mwSize nPair = 0;
    mwIndex nVert1, nVert2;
    mwIndex seg1, seg2;
	double loss=0, l=0;
    mwSize nPairFalseNeg = 0, nPairFalsePos = 0, nPairTrueNeg = 0, nPairTruePos = 0;
