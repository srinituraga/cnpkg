...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]){
	...
	for (mwIndex i=0; i<conn_num_elements; ++i) {
		if (conn_data[i] > 0)
			pqueue.push(i);
	}
	...
	while (!pqueue.empty()){
		minEdge = pqueue.top(); pqueue.pop();
		e = minEdge/nVert; v1 = minEdge%nVert; v2 = v1+nHood[e];

		if ((v2 < 0) || (v2 >= nVert))
			continue;
//		if ( (0==(v2%conn_dims[0]))||(0==(v2%conn_dims[1]))||(0==(v2%conn_dims[2])) )
//			continue;
//		if ( (0==(v1%conn_dims[0]))||(0==(v1%conn_dims[1]))||(0==(v1%conn_dims[2])) )
//			continue;
