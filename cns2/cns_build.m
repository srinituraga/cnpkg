...
UserPath = fileparts(which([Package]));
if isempty(UserPath), error('cannot find "%s.m"', Package); end
...
if ~strcmp(mode, 'clean')
    def = cns_builddef(UserPath, Package);
end
...
case 'compile'
    ...
    def2 = Generate(def, 'cuda');
    ans  = Generate(def, 'cpu' );
    ...
    def = def2;
    save(UserFilePath(true, 'def.mat'), 'def');
...
function def = Generate(def, platform)

def = Preprocess(def, platform);
...
for i = 1 : numel(def.types)
    for j = 1 : numel(def.type.(def.types{i}).kernels)
        GenerateKernelDec(def, platform, i, j);
    end
end
fclose(F);
F = -1;
...
for i = 1 : numel(def.types)
    for j = 1 : numel(def.type.(def.types{i}).kernels)
        GenerateKernelDef(def, platform, i, j);
    end
end
fclose(F);
F = -1;
...
for i = 1 : numel(def.types)
    for j = 1 : numel(def.type.(def.types{i}).kernels)
        GenerateKernelRun(def, platform, i, j);
    end
end
fclose(F);
F = -1;
...
for i = 1 : numel(def.types)
    ...
    for j = 1 : numel(d.kernels)

        ker = d.kernels{j};

        if numel(d.kernels) == 1
            head = sprintf('**  TYPE: %s  **', name);
        else
            head = sprintf('**  TYPE: %s  (KERNEL: %s) **', name, ker);
        end
        ...
        for k = 1 : numel(Help.sym)
            if (k > 1) && ((Help.type(k - 1) ~= Help.type(k)) || (Help.num(k - 1) ~= Help.num(k)))
                fprintf(F, '\n');
            end
            fprintf(F, '%s\n', Help.sym{k});
        end
...
function def = Preprocess(def, platform)
...
for i = 1 : numel(def.types)
    ...
    for j = 1 : numel(d.kernels)
        ...
        ki = cns_preprocess(Package, d.typePath, d.abstract, ker, -1);

        % Note: specifying the null and blockSize properties in "m" files is deprecated.

        if isempty(e.null)
            e.null = (ki.pmode == 'n');
        elseif isequal(e.null, false)
            if ki.pmode == 'n', error('type "%s", kernel "%s": null kernel', name, ker); end
        end

        if isempty(ki.id)
            e.baseNo = 0;
        else
            [ans, ind] = ismember(ki.id, list);
            if ind == 0
                list{end + 1} = ki.id;
                ind = numel(list);
            end
        ...
        if isempty(e.blockSize), e.blockSize = ki.blockSize; end
        if ~HelpMode && (e.kerNo > 0) && isempty(e.blockSize)
            error('type "%s", kernel "%s": block size is undefined', name, ker);
        end
        ...
        if d.dimNo == 0
            ...
            try
                e.iter = cns_transparse('iter', d, ki.iter);
            catch
                error('type "%s", kernel "%s": %s', name, ker, cns_error);
            end
        end
...
for i = 1 : numel(def.types)
    ...
    for j = 1 : numel(d.kernels)
        name = d.kernels{j};
        e = d.kernel.(name);
        if isempty(e.iter), continue; end
        key = sprintf('k%u.%u', e.baseNo, d.dimNo);
        if ~ismember(key, rsList)
            WriteKernelTrans(d, e, platform, head);
            rsList{end + 1} = key;
        end
    end
...
if ~head
    ...
    cns_transgen('init' , d, @printf, platform, fname);
end
...
function WriteKernelTrans(d, e, platform, head)

fname = sprintf('_KTrans%us%u', e.baseNo - 1, d.dimNo - 1);

if ~head
    cns_transgen('select', d, @printf, platform, fname, e.iter);
end
...
function GenerateKernelDec(def, platform, i, j)
...
if e.kerNo == 0, return; end

printf('#define _USER_KERNEL_NAME _kernel_%s_%s\n', name, ker);
...
filePath = fullfile(UserPath, sprintf('%s.h', Package));
...
function GenerateKernelDef(def, platform, i, j)
...
if ~HelpMode && (e.kerNo == 0), return; end

nd = numel(d.dims);
kerFilePath = UserFilePath(false, '%s_def_%s_%s.h', platform, name, ker);
a = ArrayInfo(e, platform);
...
for k = 1 : numel(a.names)
    printf(9998, 0, '#define %s(E) %s(%u,E)\n', upper(a.names{k}), a.macros{k}, a.offsets(k));
end

if nd == 1
    printf('#define _ALLCOORDS _c0\n');
else
    printf('#define _ALLCOORDS _c0%s\n', sprintf(', _c%u', 1 : nd - 1));
end

if ~isempty(e.iter)
    printf(9999, 0, '#define SELECT_CELL(...) _SetCoords(%u,%u,__VA_ARGS__)\n', e.baseNo - 1, d.dimNo - 1);
end

printf('#define _USER_KERNEL_NAME _kernel_%s_%s\n', name, ker);
printf('#define _USER_KERNEL_NAME2 _kernel2_%s_%s\n', name, ker);
...
printf('#undef _ALLCOORDS\n');

if ~isempty(e.iter)
    printf('#undef SELECT_CELL\n');
end

if e.kerNo == 0, return; end

f = fopen(kerFilePath, 'w');
if f < 0, error('unable to write to "%s"', kerFilePath); end

for k = 1 : nd
    fprintf(f, 'int _c%u;\n', k - 1);
end

fprintf(f, '_GetCoords(%u);\n', d.dimNo - 1);

try
    cns_preprocess(Package, d.typePath, d.abstract, ker, f);
catch
    fclose(f);
    rethrow(lasterror);
end
...
WriteCoords(q, '', struct, 'THIS_%C', false, '_c%D');
...
for j = 1 : numel(q.d.dims)
    ...
    WriteField(q, '', struct, 'THIS_%1_CENTER', false, '_GetCenter(%2,%3)', upper(dname), j - 1, pos);
    if q.d.dmap(j) == 2
        pos = q.d.sym.([dname '_shift']).pos - 1;
        WriteField(q, '', struct, 'THIS_%1_NEW', false, '_GetCoordNew(%S,%2,%3)', upper(dname), j - 1, pos);
    end
end
...
if q.d.dimTypeNo == q.typeNo
    ...
    for j = 1 : numel(q.dc.dims)
        ...
        a  = {upper(dn), d1, p1, s2, d2, p2};
        aa = {upper(dn), s2, d2, p2};

        % Old names (deprecated).
        WriteField(q, '', struct, '@GET_%T_%1_RF_DIST(Z,R,...)'     , false, '_GetRFDist(%Z,%2,%3,%4,%5,%6,R,%A)', a {:});
        WriteField(q, '', struct, '@GET_%T_%1_RF_NEAR(Z,N,...)'     , false, '_GetRFNear(%Z,%2,%3,%4,%5,%6,N,%A)', a {:});
        ...
        % New names.
        WriteField(q, '', struct, 'FIND_%T_%1_WITHIN(Z,R,...)'      , false, '_GetRFDist(%Z,%2,%3,%4,%5,%6,R,%A)', a {:});
        WriteField(q, '', struct, 'FIND_%T_%1_NEAREST(Z,N,...)'     , false, '_GetRFNear(%Z,%2,%3,%4,%5,%6,N,%A)', a {:});
        WriteField(q, '', struct, 'FIND_%T_%1_WITHIN_AT(Z,P,R,...)' , false, '_GetRFDistAt(%Z,%2,%3,%4,P,R,%A)'  , aa{:});
        WriteField(q, '', struct, 'FIND_%T_%1_NEAREST_AT(Z,P,N,...)', false, '_GetRFNearAt(%Z,%2,%3,%4,P,N,%A)'  , aa{:});
...
for i = 1 : numel(d.dims)
    ...
    if ~isempty(strfind(sym, '%C'))
        sym = strrep(sym, '%C', upper(d.dnames{i}));
    end

    if ~isempty(strfind(rep, '%D'))
        rep = strrep(rep, '%D', sprintf('%u', i - 1));
    end
...
function GenerateKernelRun(def, platform, i, j)
...
function a = ArrayInfo(e, platform)
...
for i = 1 : numel(a.names)
    ...
    elements(i) = e.array.(a.names{i}).size;
    intFlags(i) = e.array.(a.names{i}).int;
...
if ~isempty(strmatch('#define @', line))
    line(9) = '';
    sym = '';
elseif ~isempty(strmatch('#undef @', line))
    line(8) = '';
    sym = '';
elseif ~isempty(regexp(line, '^#define [^_].*', 'once'))
...
if ~KeepGenerated
    list = dir(fullfile(UserPath, [Package '_generated_*']));
    for i = 1 : numel(list)
...
func = [Package '_compiled_' platform];
...
filePath = fullfile(UserPath, [Package '_' prefix '_' sprintf(fileName, varargin{:})]);
