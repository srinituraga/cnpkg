...
if isempty(State)
    ...
    State.tic   = [];
    State.timer = timer('TimerFcn', @(obj,event)cns('admin','timer'), ...
        'ObjectVisibility', 'off', 'ExecutionMode', 'fixedDelay', 'TasksToExecute', inf, ...
        'StartDelay', 5 * 60, 'Period', 5 * 60);
    start(State.timer);
...
if isempty(FTable)
    ...
    FTable.platform = {@Platform, true };
    FTable.init     = {@Init    , true };
    FTable.test     = {@Test    , false};
    FTable.done     = {@Done    , true };
    FTable.run      = {@Run     , true };
    FTable.step     = {@Step    , true };
    FTable.output   = {@Output  , false};
    FTable.get      = {@Get     , false};
    FTable.update   = {@Update  , false};
    FTable.set      = {@Set     , true };
    FTable.shift    = {@Shift   , true };
    FTable.wait     = {@Wait    , false};
    FTable.poll     = {@Poll    , false};
    FTable.sessions = {@Sessions, false};
    FTable.devcount = {@DevCount, false};
    FTable.use      = {@Use     , true };
    FTable.admin    = {@Admin   , true };
...
try
    if isfield(FTable, mode)
        f = FTable.(mode);
        if f{2}
            [State, varargout{1 : nargout}] = f{1}(State, sid, varargin{:});
        else
            [varargout{1 : nargout}] = f{1}(State, sid, varargin{:});
        end
    else
        error('invalid mode');
    end
...
function [State, varargout] = Init(State, sid, m, varargin)
...
G = GetSession(State, sid2);

[G, err, m] = Init2(State, G, m, varargin{:});
...
[G, err, ans, dump] = Init2(State, G, m, varargin{:});
...
if nargin < 3
    if isequal(sids, -1), sids = State.use; end
else
...
function [G, err, m, dump] = Init2(State, G, m, varargin)
...
try
    ...
    if isempty(G.obj)
        ...
        g.nice     = p.nice;
        g.debug    = p.debug;
        g.funcName = [m.package '_compiled_' p.platform];
    ...
    if ~isempty(G.def.init)
        m = feval([m.package '.' G.def.init], m);
    end
...
if isempty(h.sn)
    ...
    h.sh.maxShift(1) = m.([h.sn '_maxShift']);
...
for z = 1 : numel(def.layers)
    ...
    if z == 1
        usingSteps = isfield(p, 'stepNo');
    elseif isfield(p, 'stepNo') ~= usingSteps
    ...
    if isfield(p, 'stepNo')
        ...
        if isfield(p, 'kernel')
            b = p.kernel(:)';
            if (numel(b) ~= numel(r)) || ~iscellstr(b), error('z=%u: invalid kernel', z); end
            b(strcmp(b, '')) = {'dflt'};
            [ans, b] = ismember(b, d.kernels);
            if any(b == 0), error('z=%u: invalid kernel', z); end
        else
            b = ones(1, numel(r));
        end
    else
        if isfield(p, 'kernel'), error('z=%u: kernel is invalid without stepNo', z); end
        if d.kernel.dflt.kerNo == 0
            r = [];
            b = [];
        else
            r = 1;
    ...
    r = r(inds);
    b = b(inds);
    if size(unique([r' b'], 'rows'), 1) ~= numel(r), error('z=%u: stepNo/kernel values must be unique', z); end
    for i = 1 : numel(r)
        ker = d.kernels{b(i)};
        if d.kernel.(ker).kerNo == 0, error('z=%u: invalid stepNo', z); end
    end

    for i = 1 : numel(r)
        keys(end + 1, :) = [r(i), d.typeNo, b(i), ...
...
for k = 1 : numel(rs)
    ...
    if mod(max(e.blockSize(1), 0.5), G.pr.blockYSizeAlign) ~= 0
        error('type "%s", kernel "%s": block y size must be a multiple of %u', d.type, ker, G.pr.blockYSizeAlign);
    end
    if mod(max(prod(e.blockSize), 0.5), G.pr.blockSizeAlign) ~= 0
        error('type "%s", kernel "%s": block size must be a multiple of %u', d.type, ker, G.pr.blockSizeAlign);
    end
    if prod(e.blockSize) < G.pr.minBlockSize
        error('type "%s", kernel "%s": block size must be at least %u', d.type, ker, G.pr.minBlockSize);
    end
    ...
    c.stepNo    = rs(k);
    c.kerNo     = e.kerNo;
    c.blockSize = prod(e.blockSize);
    c.z0        = zs{k}(1);

    blocks = zeros(8, 0, ns(k), 'uint16');

    for z = zs{k}

        q = s.layers{z};

        if (q.yCount == 0) || (q.xCount == 0), continue; end
        ...
        if isempty(e.iter)
            ...
            keep = true(1, numel(q.t.siz2));
            keep(e.iter) = false;
            keep = find(keep(q.t.perm));
            keepy = keep(keep <= q.t.nparts(1));
            keepx = keep(keep >  q.t.nparts(1));
            if any(keepy == 1)
                yCount0 = q.t.siz3 (1);
                ySize0  = q.t.siz3a(1);
            else
            ...
            yCount = prod(q.t.siz3a(keepy));
            xCount = prod(q.t.siz3a(keepx));
            if isempty(keepy), yStep = 1; else yStep = prod(q.t.siz3a(1                 : keepy(1) - 1)); end
            if isempty(keepx), xStep = 1; else xStep = prod(q.t.siz3a(q.t.nparts(1) + 1 : keepx(1) - 1)); end
        end
        ...
        if ~isfield(m, 'quiet') || ~m.quiet
            if (yCountOpt < G.pr.blockYSizeAlign) && (xCount >= 2 * G.pr.blockYSizeAlign)
                fprintf('Warning: z=%u: for thin layers, yCount > xCount is more efficient\n', z);
            end
        end

        blockYSize = OptimizeBlock(e.blockSize(1), e.blockSize(2), G.pr.blockYSizeAlign, yCountOpt, xCount);
        blockXSize = prod(e.blockSize) / blockYSize;
        ...
        xs = cumsum([0 xc(1 : end - 1)]) * xStep;

        [xsGrid, ysGrid] = meshgrid(xs, ys);
        xcGrid = repmat(xc , numel(ys), 1);
        ycGrid = repmat(yc', 1, numel(xs));

        blocks2 = zeros(8, numel(xsGrid) / ns(k), ns(k), 'uint16');
        blocks2(1, :, :) = reshape(xsGrid, 1, [], ns(k));
        blocks2(2, :, :) = reshape(ysGrid, 1, [], ns(k));
        blocks2(3, :, :) = z - 1;
        blocks2(4, :, :) = blockYSize;
        blocks2(5, :, :) = reshape(xcGrid, 1, [], ns(k));
        blocks2(6, :, :) = reshape(ycGrid, 1, [], ns(k));
        blocks2(7, :, :) = xStep;
        blocks2(8, :, :) = yStep;
    ...
    c.bOff   = numel(h.dBlocks) / 8;
    c.bCount = numel(blocks) / 8;
...
[ans, ans, p] = GetAllParams(G, 'r', args{:});
...
function [State, varargout] = Step(State, sid, step, varargin)
...
if any(mod([step1 step2], 1) ~= 0), error('invalid step'); end
if (step1 < 1) || (step2 < step1 - 1), error('invalid step'); end
...
[ans, n, p] = GetAllParams(G, 'g', varargin{:});

if nargout ~= n, error('%u outputs needed', n); end
...
for i = 1 : numel(p)
    if isempty(p(i).varType)
        ...
        a = G.func(6, G.obj, p(i));
        a = GetFinalize(p(i), a, false);
    end
    if p(i).zNo == 0
        varargout{p(i).rNo} = a;
    else
        varargout{p(i).rNo}{p(i).zNo} = a;
    end
end
...
if isequal(sid, -1), sid = State.use; end
...
[G, ans, p, a] = GetAllParams(G, 's', varargin{:});
...
function [G, n, p, a] = GetAllParams(G, op, varargin)
...
for i = 1 : n
    ...
    arg = args{i};
    if ~iscell(arg), error('invalid argument'); end
    if isempty(arg), error('incorrect number of arguments'); end
    z = arg{1};

    if ischar(z)
        [p{end + 1}, a{end + 1}] = GetHostParams(op, arg{:});
        p{end}.rNo = i;
        p{end}.zNo = 0;
    elseif z >= 0
        [G, p{end + 1}, a{end + 1}] = GetParams(G, op, arg{:});
        p{end}.rNo = i;
        p{end}.zNo = 0;
    else
        if op == 'r', error('a group number cannot be used in this call'); end
        if -z > G.def.gCount, error('invalid group number'); end
        z = G.def.groups{-z}.zs;
        if op == 's'
            if numel(arg) < 2, error('incorrect number of arguments'); end
            if ~iscell(arg{end}), error('invalid argument'); end
            if numel(arg{end}) ~= numel(z), error('incorrect number of arguments'); end
        end
        for j = 1 : numel(z)
            if op == 'g'
                [G, p{end + 1}, a{end + 1}] = GetParams(G, op, z(j), arg{2 : end});
            else
                [G, p{end + 1}, a{end + 1}] = GetParams(G, op, z(j), arg{2 : end - 1}, arg{end}{j});
            end
            p{end}.rNo = i;
            p{end}.zNo = j;
        end
...
function [G, p, a] = GetParams(G, op, z, fieldName, varargin)
...
if isempty(name) || (name(1) ~= '$')
    % This is because we might enhance get/set/etc to allow group/layer names.
    error('property "%s": name must start with "$"', name);
end
...
if (numel(inc) > 1) || any(inc < 0), error('invalid shift value'); end
...
elseif inc <= sh.maxShift

    sh.current = sh.current + inc;

    sh.shift = sh.flag .* max(1 + floor((sh.current - sh.lag - sh.start) ./ sh.space + 0.001) - sh.size, 0);
    if any(sh.shift > sh.size)
        % Should not be possible if proper dimension mapping was used.
        % Note that this is not the only problem maxShift guards against.
        error('attempt to shift a layer by more than its size');
    end
    ...
    error('maximum shift is %u, %u attempted', sh.maxShift, inc);
...
function [State, out] = Use(State, sid, flag)

if (nargin < 3) && isequal(sid, -1)
    out = State.use;
    if isequal(out, -1), out = []; end
    out = uint32(out);
    return;
end
...
if nargin < 3
    if isempty(sid)
        sid = -1;
    elseif ~isscalar(sid) || (sid == 0)
        error('invalid session id');
    end
else
    if ~isequal(sid, -1), error('too many arguments'); end
    if ~strcmp(flag, 'none'), error('invalid parameter'); end
end
...
function State = Admin(State, sid, mode)
...
case 'timer'
    ...
    if ~isempty(State.tic) && (toc(State.tic) > 15 * 60)
        if ~isempty(AllSessions(State))
            fprintf('cns: closing inactive session(s)\n');
        end
        State = Done(State, sid, 'all');
        State.tic = [];
    end
