...
% 	for k=1:3,
% 		idx{k} = (m.leftBorder(k)+half_big_output_size(k)+1):(imSz{imPickNeg}-m.rightBorder(k)-half_big_output_size(k));
% 	end
% 	%wt = edge{imPickNeg}{segPickNeg}(idx{1},idx{2},idx{3});
% 	wt = mask{imPickNeg}{segPickNeg}(idx{1},idx{2},idx{3});
% 	if ~sum(wt(:)), continue, end
% 	[index_bigneg(1) index_bigneg(2) index_bigneg(3)] = ind2sub(imSzValid{k},randsample(wt(:),1,1));
	for k=1:3,
