...
% 	connNeg = MakeConnLabel(segNeg.*maskNeg,m.params.nhood);
	...
	[segNeg,cmpsSzNeg] = connectedComponents( ...
						MakeConnLabel( ...
					seg{imPickNeg}{segPickNeg}(idxGraphOutNeg{1},idxGraphOutNeg{2},idxGraphOutNeg{3}), ...
					m.params.nhood),m.params.nhood,[],0);
	segNeg = single(segNeg); cmpsSzNeg = double(cmpsSzNeg);
	maskNeg = mask{imPickNeg}{segPickNeg}(idxGraphOutNeg{1},idxGraphOutNeg{2},idxGraphOutNeg{3});
	cmpsNeg = unique(segNeg.*maskNeg);
	cmpsNeg = cmpsNeg(cmpsNeg~=0);
	% reject patch if it doesn't have more than one object!
...
% fprintf('everything in between...'),tic
	...
	for iex = 1:m.params.minibatch_size,
		tryAgain = 1; nTries = 0;
		while tryAgain,
			cmpPick = randsample(cmpsNeg,1,true,cmpsSzNeg(cmpsNeg));
			idxpt1 = find(maskNeg(:).*(segNeg(:)==cmpPick));
			[pt1(1) pt1(2) pt1(3)] = ind2sub(m.params.graph_size,idxpt1(randi(length(idxpt1))));
			if sum(cmpsNeg~=cmpPick)~=1,
				cmpPick = randsample(cmpsNeg(cmpsNeg~=cmpPick),1,true,cmpsSzNeg(cmpsNeg(cmpsNeg~=cmpPick)));
			else,
				cmpPick = cmpsNeg(cmpsNeg~=cmpPick);
			end
			idxpt2 = find(maskNeg(:).*(segNeg(:)==cmpPick));
			[pt2(1) pt2(2) pt2(3)] = ind2sub(m.params.graph_size,idxpt2(randi(length(idxpt2))));
