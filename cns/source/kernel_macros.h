...
***********************************************************************************************************************/
    ...
    #define _SELECT_SYN(E) \
        const unsigned int _si = _CheckSyn(_session, _sc, E, _z, _y, _x); \
        const ushort4      _sm = _p.m_smPtr[(_si * _p.m_xCount + _x) * _p.m_ySize + _y];
...
#define _GetLayerSz(S,D)           _LTrans##S##_s(_LTPtr(_z,_LT_SIZ2P), D)
#define _GetZLayerSz(Z,S,D)        _LTrans##S##_s(_LTPtr(Z,_LT_SIZ2P) , D)

#define _GetCoord(S,D)             _LTrans##S##_c##D(_LTPtr(_z,_LT_SIZ2P)   , _y   , _x   )
...
#define _GetHNFieldSz(H,B,D)       _NTrans##B##_sh(H, D)
...
#define _GetHWFieldSz(H,B,D)       _WTrans##B##_sh(H, D)
...
#define _GetCoordNew(S,D,F) (_GetCoord(S,D) - (_GetLayerSz(S,D) - __float_as_int(_GetLConst(F))))
