...
INLINEH unsigned int _Unroll(unsigned int c, unsigned int roll, unsigned int size);
...
public:
    ...
    const ushort4      *GetDBlocks  (unsigned int offset) { return m_dBlocks   + offset; };

    _LayerData *GetDLayers(unsigned int z) { return m_dLayers + z; };

    const unsigned int *GetHKernelZs(unsigned int offset) { return m_hKernelZs + offset; };
...
bool _Session::Init(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    w.dBlocksPtr   = (const ushort4      *)mxGetData(mxGetField(h, 0, "dBlocks"  ));
    ...
    w.dBlocksNum   = mxGetNumberOfElements(mxGetField(h, 0, "dBlocks"  )) / 4;

    Worker(&_Session::Init2, &w, true);

    const mxArray *t = mxGetField(h, 0, "hKernelZs");
    _AllocHostBuf(this, m_hKernelZs, (const unsigned int *)mxGetData(t), mxGetNumberOfElements(t), "kernel zs");
...
void _Layer::LInit(_Session *s, const mxArray *layers, unsigned int z) {
    ...
    m_xShift     = (unsigned int)mxGetScalar(mxGetField(layers, z, "xShift"    ));
...
void _Kernel::KInit(_Session *s, const mxArray *kernels, unsigned int k) {
    ...
    m_zPtr = m_s->GetHKernelZs((unsigned int)mxGetScalar(mxGetField(kernels, k, "zOff")));
...
/**********************************************************************************************************************/
    ...
    void _Layer::LRun(unsigned int phase) {
        ...
        unsigned int xStart, xCount;
        if (m_shLFlag) {
            // Find which columns to compute based on the shift size & roll value.
            if (m_shift == 0) return;
            unsigned int size = m_xCount / m_xShift;
            if (m_shift < size) {
                xStart = m_xShift * _Roll(size - m_shift, m_roll, size);
                xCount = m_xShift * m_shift;
            } else {
                xStart = 0;
                xCount = m_xCount;
            }
        } else {
            // No shiftable dimension.  Compute all columns.
            xStart = 0;
            xCount = m_xCount;
        }
        ...
        default:
            m_s->Exit2("invalid type number (%u)", m_typeNo + 1);
            break;
        }
...
/**********************************************************************************************************************/
    ...
    void _Kernel::KRun() {
        ...
        if (m_shKFlag) {
            ...
            unsigned int shift = m_s->GetLayer(m_zPtr[0])->GetShift();
            unsigned int roll  = m_s->GetLayer(m_zPtr[0])->GetRoll ();
            if (shift == 0) return;
            if (shift < size) {
        ...
        // The particular 2D shapes of the grid and blocks are irrelevant.
    ...
    void _Kernel::KRun() {
        ...
        for (unsigned int i = 0; i < m_zCount; i++) {
            m_s->GetLayer(m_zPtr[i])->LRun(m_phase);
        }
