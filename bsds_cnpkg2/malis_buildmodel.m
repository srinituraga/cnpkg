...
% big fwd pass
l = l(end) + 1;
m.layer_map.big_minibatch_index = l;
l = l(end) + 1;
m.layer_map.big_input = l;
for h = 1:(m.params.num_layers-1),
	l = l(end) + (1:length(m.params.layer{h}.nHid));
	m.layer_map.big_hidden{h} = l;
end
l = l(end) + 1;
m.layer_map.big_output = l;
l = l(end) + 1;
m.layer_map.big_label = l;
m.layer_map.big_hidden_prev{1} = m.layer_map.big_input;
for h = 2:length(m.layer_map.big_hidden),
	m.layer_map.big_hidden_prev{h} = m.layer_map.big_hidden{h-1};
end
m.layer_map.big_hidden_next = {};
for h = 1:length(m.layer_map.big_hidden)-1,
	m.layer_map.big_hidden_next{h} = m.layer_map.big_hidden{h+1};
end
m.layer_map.big_hidden_next{end+1} = m.layer_map.big_output;
...
% big minibatch index layer
l = m.layer_map.big_minibatch_index;
m.layers{l}.name = 'big index';
m.layers{l}.type = 'index';
m.layers{l}.stepNo  = [];
m.layers{l}.size{1} = 5;
m.layers{l}.size{2} = 1;
% input layer
l = m.layer_map.big_input;
m.layers{l}.z       = l; % For display only.
m.layers{l}.name    = 'big inputLayer';
m.layers{l}.type    = 'input';
m.layers{l}.stepNo  = [];
m.layers{l}.size{1} = m.params.input_units;
m.layers{l}.zin		= m.layer_map.big_minibatch_index;
% hidden layers
for i = 1:length(m.layer_map.big_hidden),
	for s = 1:length(m.layer_map.big_hidden{i}),
		l = m.layer_map.big_hidden{i}(s);
		m.layers{l}.z       = l; % For display only.
		m.layers{l}.name    = ['big hiddenLayerNo ' num2str(i) ', scaleNo ' num2str(s)];
		m.layers{l}.type    = 'hidden';
		m.layers{l}.stepNo  = [];
		m.layers{l}.zp      = m.layer_map.big_hidden_prev{i};
		m.layers{l}.zw      = m.layer_map.weight{i}{s};
		m.layers{l}.zb      = m.layer_map.bias{i}{s};
		m.layers{l}.znw     = m.layer_map.weight_out{i}{s};
		m.layers{l}.zn      = m.layer_map.big_hidden_next{i};
		m.layers{l}.size{1} = m.params.layer{i}.nHid{s};
	end
end
% output layer
l = m.layer_map.big_output;
m.layers{l}.z       = l; % For display only.
m.layers{l}.name    = sprintf('big output');
m.layers{l}.type    = 'output';
m.layers{l}.stepNo  = [];
m.layers{l}.zp      = m.layer_map.big_hidden{end};
m.layers{l}.zw      = m.layer_map.weight{end}{1};
m.layers{l}.zb      = m.layer_map.bias{end}{1};
m.layers{l}.size{1} = m.params.output_units;
m.layers{l}.zl		= m.layer_map.label;
% label layer
l = m.layer_map.big_label;
m.layers{l}.z       = l; % For display only.
m.layers{l}.name    = ['big label'];
m.layers{l}.type    = 'label';
m.layers{l}.stepNo  = [];
m.layers{l}.size{1} = m.params.output_units;
m.layers{l}.zin		= m.layer_map.minibatch_index;
