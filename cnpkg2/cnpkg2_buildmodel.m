...
for i = 1:length(m.layer_map.hidden),
	for s = 1:length(m.layer_map.hidden{i}),
		...
		for sp = 1:length(m.layer_map.hidden_prev{i}),
			...
			m.layers{l}.val		= upSmpl(dnSmplMean(m.layers{l}.val,[1 m.layers{l}.wPxSize 1]),[1 m.layers{l}.wPxSize 1]);
		end
...
% gradient pass
% weights get two steps (one for computing the gradient and one for applying constraints and updating the weights)
% biases get only one step (since there are no constraints to apply)
step = step+1;
