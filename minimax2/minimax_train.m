function m = minimax_train(m,show_plot,platform,nice,skip_gpu_init)
...
if ~isfield(data,'weight'),
    data.weight = ones(1,length(data.im))/length(data.im);
end
...
while(m.stats.iter < m.params.maxIter),
	...
	% % reject patch if it doesn't have boundary pixels & has only one object!
	% if (cmpsNeg(1)~=0 && length(cmpsNeg)==1),
	% 	if m.params.debug, disp('Negative patch doesn''t have more than one object!'), end
	% 	continue
	% end
...
% fprintf('Running big fwd pass on gpu...'),tic
    ...
    bmapEst = cns('step',[1 m.layers{m.layer_map.output}.stepNo(end)],{m.layer_map.output,'val'});
	bmapEst = permute(bmapEst,[2 3 4 1 5]);
% fprintf(' done. '),toc
% fprintf('everything in between...'),tic
    ...
    [dlossNeg,loss(1),classerr(1),randIndex(1)] = minimax_loss( ...
                                                bmapEst(:,:,:,:,1), ...
                                                segNeg, ...
                                                m.layers{m.layer_map.error}.param, ...
                                                false);
    [dlossPos,loss(2),classerr(2),randIndex(2)] = minimax_loss( ...
                                                bmapEst(:,:,:,:,2), ...
                                                segPos, ...
                                                m.layers{m.layer_map.error}.param, ...
                                                true);
    % multiply by the derivative of the output nonlinearity (this is not done in the CNS package)
    dlossNeg = dlossNeg .* bmapEst(:,:,:,:,1).*(1-bmapEst(:,:,:,:,1));
    dlossPos = dlossPos .* bmapEst(:,:,:,:,2).*(1-bmapEst(:,:,:,:,2));
...
% fprintf('done. '),toc
	...
	if ~rem(m.stats.iter,m.params.nIterPerEpoch*m.params.nEpochPerSave) || (m.stats.iter==1 && m.stats.epoch==1),
...
for imIdx = 1:m.params.nDataBlock,
	imPick = randsample(length(data.weight),1,true,data.weight);
